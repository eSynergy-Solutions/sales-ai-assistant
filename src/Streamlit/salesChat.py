...
from streamlit_support import check_password, get_remote_ip, create_markdown, _submit_feedback, set_header, get_button_links
...
try:
    collector = FeedbackCollector(email=os.getenv("STREAMLIT_FEEDBACK_EMAIL"),
...
st.set_page_config(
    ...
    page_icon=
    "https://esynergy.co.uk/wp-content/uploads/2022/02/cropped-fav-icon-32x32.png",
    layout="wide",
...
for n, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        if "button_source" in message and len(
                list(message["button_source"].keys())) > 1:
            selected = None
            selected = pills("Source",
...
if prompt := st.chat_input("What is up?"):
    ...
    with st.chat_message("assistant"):
        ...
        # }
        with st.spinner(
                'Fetching the Response for your Query from the AI Engine...'):
        ...
        try:
            ...
            selected_current = pills("Source",
                                     list(button_links.keys()),
                                     key=next(widget_id))
